...
func (socket *mongoSocket) getnone() (nonce string, err error) {
	socket.Lock()
...
func (socket *mongoSocket) resetNonce() {
	...
	op := &queryOp{}
	op.query = &getnone{GetNonce: 1}
	op.collection = "admin.$cmd"
	op.limit = -1
	op.replyFunc = func(err error, reply *replyOp, docNum int, docData []byte) {
		if err != nil {
			socket.kill(errors.New("getNone: "+err.Error()), true)
			return
		}
...
func (socket *mongoSocket) loginClassic(cred Credential) error {
	...
	cmd := authenticate{Authenticate: 1, User: cred.Username, Nonce: nonce, Key: key}
	res := authResult{}
